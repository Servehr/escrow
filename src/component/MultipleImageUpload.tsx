...
export default function MultipleImageUpload({width, ICloudColour, allowedFileTypes, onClick}: ImageProps) 
{
    ...
    return (
          <>
              <div 
                  className="theArea mb-2 flex-col w-7/12 md:w-4/12 object-fill rounded-md px-10 py-1 flex justify-center items-center mx-auto cursor-pointer hover:text-green-300 uploader md:bg-[#f0f8ff] md:border-2 md:border-gray-200"
                  onClick={() => { 
                    hover() 
                  }}
              >                 
                    ...
                    <div 
                        className={`passportText ${ICloudColour} w-col-span-12 md:ml-28 md:py-5 d-flex justify-center mb-4 text-center mx-auto font-bold upload`}
                    > 
              ...
              <div 
                    className="p-3 w-full md:grid md:grid-cols-12 md:gap-5 md:mt-10"
              >
                {
                    previewUrls && previewUrls.map((image, index) => {
                        return (
                                <div 
                                    className="md:col-span-3 w-full z-30 border border-4 border-blue-200 mb-3 md:mb-0 h-23 relative" key={index}
                                >
                                    <img src={image} alt="upload" className='col-span-12' />
                                    <div 
                                        className={`absolute flex justify-between bottom-0 w-full pl-2 pb-3`}
                                    >
